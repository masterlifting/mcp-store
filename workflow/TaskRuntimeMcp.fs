module TaskRuntimeMcp

// Native MCP transport for the existing Task Runtime. The host owns only the
// JSON-RPC/DTO boundary; all task semantics, CAS, persistence, and path safety
// remain in TaskRuntime.
...
let private tools =
     """[{"name":"task_create","description":"Create a Task Runtime task using the strict schema-v3 DTO boundary.","inputSchema":{"type":"object","additionalProperties":false,"required":["projectRoot","taskId","title","kind","acceptanceCriteria","workItems"],"properties":{"projectRoot":{"type":"string"},"taskId":{"type":"string"},"title":{"type":"string"},"kind":{"type":"string","enum":["execution","research"]},"profile":{"type":"string"},"acceptanceCriteria":{"type":"array"},"workItems":{"type":"array"}}}},{"name":"task_get","description":"Read a Task Runtime task without changing state.","inputSchema":{"type":"object","additionalProperties":false,"required":["projectRoot","taskId"],"properties":{"projectRoot":{"type":"string"},"taskId":{"type":"string"}}}},{"name":"task_apply","description":"Apply one existing Task Runtime command with expectedStateRevision CAS. Authority is always the trusted coordinator invocation; authority or receipt fields are not accepted.","inputSchema":{"type":"object","additionalProperties":false,"required":["projectRoot","taskId","expectedStateRevision","command"],"properties":{"projectRoot":{"type":"string"},"taskId":{"type":"string"},"expectedStateRevision":{"type":"integer","minimum":0},"command":{"type":"object"}}}},{"name":"task_validate","description":"Validate a persisted Task Runtime task and report contract/profile drift.","inputSchema":{"type":"object","additionalProperties":false,"required":["projectRoot","taskId"],"properties":{"projectRoot":{"type":"string"},"taskId":{"type":"string"}}}}]"""
...
let private initializeResult protocolVersion =
    ...
    serverInfo["name"] <- node "opencode-task-runtime"
...
let private validateCatalog args =
    ...
    | _ -> Error "the Task Runtime requires --profile-catalog <absolute path> --profile-catalog-sha256 <sha256>"

[<EntryPoint>]
let main args =
    match validateCatalog args with
    | Error message ->
        Console.Error.WriteLine($"task-runtime startup failed: {message}")
        1
    | Ok() ->
