...
class GetForumTopicsByIDRequest(TLRequest):
    """Raw request for channels.getForumTopicsByID missing in Telethon 1.42-1.43."""

    CONSTRUCTOR_ID = 0xB0831EB9
    SUBCLASS_OF_ID = 0x0

    def __init__(self, channel, topics):
        self.channel = channel
        self.topics = topics
    ...
    def _bytes(self):
        return b"".join(
            (
                struct.pack("<I", self.CONSTRUCTOR_ID),
                self.channel._bytes(),
                struct.pack("<I", 0x1CB5C415),
                struct.pack("<i", len(self.topics)),
                b"".join(struct.pack("<i", topic) for topic in self.topics),
            )
        )
...
async def list_topics(
    ...
    if isinstance(offset_topic, bool) or not isinstance(offset_topic, int) or offset_topic < 0:
        return "offset_topic must be a non-negative integer."
    if (
        isinstance(limit, bool)
        or not isinstance(limit, int)
        or not 1 <= limit <= MAX_FORUM_TOPICS_PAGE_SIZE
    ):
        return f"limit must be an integer between 1 and {MAX_FORUM_TOPICS_PAGE_SIZE}."
    try:
        cl = get_client(account)
        ...
        for topic in topics:
            ...
            record = {
                ...
                "read_through_message_id": _integer_or_zero(
                    getattr(topic, "read_inbox_max_id", None)
                ),
                "latest_message_id": _integer_or_zero(getattr(topic, "top_message", None)),
            }
    ...
    **Performance:** when `with_about=True`, makes one extra API call per chat
    ...
    try:
        ...
        for dialog in dialogs:
            ...
            record["read_through_message_id"] = _integer_or_zero(
                getattr(inner_dialog, "read_inbox_max_id", None)
            )
            record["latest_message_id"] = _integer_or_zero(
                getattr(inner_dialog, "top_message", None)
            )
...
def _integer_or_zero(value) -> int:
    """Return a protocol message ID or the explicit unavailable value."""
